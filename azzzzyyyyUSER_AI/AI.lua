...
dofile("./AI_sakray/USER_AI/Const_.lua")
dofile("./AI_sakray/USER_AI/H_SkillList.lua") 
dofile("./AI_sakray/USER_AI/Defaults.lua")
dofile("./AI_sakray/USER_AI/AzzyUtil.lua")
dofile("./AI_sakray/USER_AI/Stubs.lua")
...
function WriteStartupLog(Version,ErrorCode,ErrorInfo)
	...
	if fsize("./AI_sakray/USER_AI/Const_.lua")~=ConstLen then
		ErrorCode="Damaged File"
		ErrorInfo=ErrorInfo.." Const_.lua has been modified or is corrupted"
	end
	if fsize("./AI_sakray/USER_AI/AI_main.lua")~=MainLen then
		ErrorCode="Damaged File"
		ErrorInfo=ErrorInfo.." AI_main.lua has been modified or corrupted"
	end
	--]]
	if fsize("./AI_sakray/USER_AI/H_Config.lua")==3898 then
		ConfigVers="Default: "..LastSavedDate
	else
		ConfigVers="Custom, edited "..LastSavedDate
	end
	if fsize("./AI_sakray/USER_AI/H_Tactics.lua")==5393 then
